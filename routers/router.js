...
var fs = require('fs');
var basic = require('../basicmodule/basic.js');
...
/*
以下代码含有异步操作，需要采用上面的Promise函数解决
function findID(App,idd,res) {
    App.findOne({appid:idd},function(err,app) {
      if(err) {
        return res.status(500).send('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>404 Not Found!!!</title><style>center {font-size: 44px;color: red;height: 600px;line-height: 600px;}</style></head><body><center>读取文件错误或输入的参数有误！！！</center></body></html>');
      }
      return app
...
router.get('/edit',async(req,res) => {
  try {
    ...
    console.log(idd);
    res.render('edit.html',{
      app:findID(App,idd,res),
      appComment:findID(AppComment,idd,res),
      appFeature:findID(AppFeature,idd,res),
      blog:findID(Blog,idd,res),
      faq:findID(FAQ,idd,res),
      user:req.session.user
    })
  }
  catch (e) {
...
function postEdit(App,req,res) {
  ...
  addNew(App,req,res);
  // for(let i in req.files) {
  //   req.files[i].mv('./public/images/' + req.files[i].name);
  //   app[i] = '/public/images-min/' + req.files[i].name.replace('.jpg','.webp').replace('.png','.webp');
  // }
  // App.updateMany({appid: idd},app,{upsert:true},function(err) {
  //   if(err) {
  //     return res.status(500).send('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>404 Not Found!!!</title><style>center {font-size: 44px;color: red;height: 600px;line-height: 600px;}</style></head><body><center>读取文件错误或输入的参数有误！！！</center></body></html>');
  //   }
  //   console.log(app);
...
router.post('/login',function(req,res) {
  ...
  User.findOne({email:body.email},function(err,data) {
    // console.log(data.password===md5(md5(body.password)));
    if(err) {
...
router.get('/file',function(req,res) {
  var infos = getInfos(DIR);
  var dirs = infos.dirs;
  var files = infos.files;
...
router.post('/file.html',function (req, res) {
  res.send(getInfos(DIR));
});

//delete
router.post('/deleteFile.html', function (req, res) {
...
function getInfos(DIR) {
  var dirs = basic.getFileinfos(DIR);
  // console.log(dirs);
  var files = [];
  for (let i = 0;i < dirs.length;i++) {
    files[i] = basic.getFileinfos(dirs[i].file_name+'/')
  }
  return {files, dirs}
}
